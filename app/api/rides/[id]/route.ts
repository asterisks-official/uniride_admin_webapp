...
/**
 * GET /api/rides/[id]
 * Get a single ride by ID with optional related data
 * Query params:
 * - includeTransactions: Include ride transactions (default: false)
 * - includeCancellation: Include cancellation details (default: false)
 */
export async function GET(
  ...
  try {
    ...
    const includeCancellation = searchParams.get('includeCancellation') === 'true';
    ...
    const responseData: any = { ride };

    // Optionally include transactions
    if (includeTransactions) {
      const transactions = await ridesRepo.getRideTransactions(id);
      responseData.transactions = transactions;
    }

    // Optionally include cancellation details
    if (includeCancellation) {
      const cancellation = await ridesRepo.getRideCancellation(id);
      responseData.cancellation = cancellation;
    }
