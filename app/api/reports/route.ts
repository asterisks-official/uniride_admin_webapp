...
import { reportFiltersSchema, paginationSchema } from '@/lib/validation/schemas';
import { z } from 'zod';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filtersInput = {
      ...
      reportedUserUid: searchParams.get('reportedUserUid') || undefined,
      category: searchParams.get('category') || undefined,
      severity: searchParams.get('severity') || undefined,
      reporterRole: searchParams.get('reporterRole') || undefined,
    };
    ...
    // Fetch reports
    const result = await reportsRepo.listReports(filters, pagination);
